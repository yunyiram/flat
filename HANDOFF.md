...
- 규칙 9 변형 vs 새 패턴: cascade_pattern.md #3 본체 적용 = sync_check와 분담 = **새 패턴** (변형 X). 즉 사고 9 우로보로스 X, 정상 신규 도구
...
| **누락 방지 #1 SOP 강제 호출** | flat_session_sop.md 호출 의무화 합의 필요 | — | 합의 대기 |
...
| **SOP 강제 호출 합의 제안서 (cont.75 F)** | ✅ `docs/cont75_sop_enforcement_proposal.md` 신설 (Q1-Q4 결정 input + 완전 청산 옵션 + 추천 매트릭스) | 이람 결정 (Q1-Q4 추천 채택 또는 반대) | input 완료, 결정 대기 |
