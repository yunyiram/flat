...
| 봉제 현장용어 60 | ✅ data 신설 | ✅ **cont.72 Part 13-15 — B6.5 Phase 1+2+3 implement**: Phase 1 매핑 표 (`data/factory_terms_i18n_mapping.json`, 60→18 매핑 + 26 construction-only) / Phase 2 LANG.ko_factory section (19 entries, sleeve/body/neck/detail/pants/skirt 6 카테고리) / Phase 3 "공장" 토글 버튼 + t() fallback. 병기 형식 default C (표준어 메인 + 현장어 괄호). 후속: Phase 4 PDF 통합 (factory validation 후) + 이람 brand voice 검수 + i18n 확장 6 keys (에리고시/에리구리/단작/마이깡/깡/우라). |
